...
use crate::chess::consts::{NUM_FILES, NUM_PIECES, NUM_SQUARES};
use crate::chess::types::color::Color;
use crate::chess::types::color::Color::{Black, White};
...
impl Zobrist {
    pub fn new() -> Self {
        ...
        for piece_index in 0..NUM_PIECES {
            for square_index in 0..NUM_SQUARES {
                zobrist.squares[piece_index][square_index] = rng.r#gen::<u64>();
            }
        }
        for i in 0..NUM_FILES {
            zobrist.double_jump[i] = rng.r#gen::<u64>();
        }
        for i in 0..4 {
            zobrist.castling_rights[i] = rng.r#gen::<u64>();
        }

        zobrist.side_to_move = rng.r#gen::<u64>();
    ...
    pub fn hash_from_board(&self, board: &Board) -> u64 {
        ...
        if board.has_short_castle_rights(White) {
            final_zobrist ^= self.castling_rights[WHITE_SHORT_CASTLE];
        }
        if board.has_long_castle_rights(White) {
            final_zobrist ^= self.castling_rights[WHITE_LONG_CASTLE];
        }
        if board.has_short_castle_rights(Black) {
            final_zobrist ^= self.castling_rights[BLACK_SHORT_CASTLE];
        }
        if board.has_long_castle_rights(Black) {
            final_zobrist ^= self.castling_rights[BLACK_LONG_CASTLE];
        }
