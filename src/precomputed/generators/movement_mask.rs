...
use crate::precomputed::generators::helpers::{
    INCLUDE_EDGE, create_dynamic_mask, create_static_mask,
};
    ...
    pub fn new() -> Self {
        ...
        for i in 0..NUM_SQUARES {
            ...
            if rank.is_pawn_start(Color::White) {
                masks.white_double_jump[i] |= 1 << (i + 16)
            }
            if rank.is_pawn_start(Color::Black) {
                masks.black_double_jump[i] |= 1 << (i - 16)
            }

            if !rank.is_pawn_promotion(Color::White) {
                masks.white_pawn_move[i] |= 1 << (i + 8)
            }
            if !rank.is_pawn_promotion(Color::Black) {
                masks.black_pawn_move[i] |= 1 << (i - 8)
            }

            masks.white_pawn_attacks[i] |=
                create_static_mask(&consts::WHITE_PAWN_ATTACKS_DIRECTIONS, square);
            masks.black_pawn_attacks[i] |=
                create_static_mask(&consts::BLACK_PAWN_ATTACKS_DIRECTIONS, square);
            ...
            masks.bishop[i] |=
                create_dynamic_mask::<INCLUDE_EDGE>(&consts::BISHOP_DIRECTIONS, square);
            masks.rook[i] |= create_dynamic_mask::<INCLUDE_EDGE>(&consts::ROOK_DIRECTIONS, square);
            masks.queen[i] |=
                create_dynamic_mask::<INCLUDE_EDGE>(&consts::QUEEN_DIRECTIONS, square);
        }
