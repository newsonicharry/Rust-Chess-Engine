    ...
    fn create_in_between(&mut self) {
        for i in 0..NUM_SQUARES {
            ...
            for j in 0..NUM_SQUARES {
                ...
                // diagonal
                if is_diagonal && dif_x.is_positive() && dif_y.is_positive() {
                    dir = (1, 1);
                }
                if is_diagonal && dif_x.is_negative() && dif_y.is_positive() {
                    dir = (-1, 1);
                }
                if is_diagonal && dif_x.is_positive() && dif_y.is_negative() {
                    dir = (1, -1);
                }
                if is_diagonal && dif_x.is_negative() && dif_y.is_negative() {
                    dir = (-1, -1);
                }
