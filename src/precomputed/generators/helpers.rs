...
pub fn create_dynamic_mask<const EDGE: bool>(directions: &[(i8, i8)], square: Square) -> u64 {
    ...
    for direction in directions {
        ...
        loop {
            ...
            if (EDGE == NO_EDGE)
                && ((x_dir != 0 && (new_x > 6 || new_x < 1))
                    || (y_dir != 0 && (new_y > 6 || new_y < 1)))
            {
                break;
            }
...
pub fn generate_blockers(mask: u64) -> Box<[u64]> {
    // let no_edge_mask = self.no_edge_masks[square as usize];
    let squares = bits::all_squares(mask);
    ...
    for pattern_index in 0..total_blocker_patters {
        for square_index in 0..squares.len() {
            ...
            let curr_square = squares.get(square_index).unwrap();

            all_blocker_patters[pattern_index] |= (bit << *curr_square as u64) as u64;
        }
    }
