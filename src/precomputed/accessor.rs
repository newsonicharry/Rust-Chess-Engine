...
pub static MOVEMENT_MASKS: MovementMasks =
    unsafe { std::mem::transmute(*include_bytes!("bins/movement_masks.bin")) };
pub static IN_BETWEEN: InBetween =
    unsafe { std::mem::transmute(*include_bytes!("bins/in_between.bin")) };
pub static ZOBRIST: Zobrist = unsafe { std::mem::transmute(*include_bytes!("bins/zobrist.bin")) };
pub static LMR_REDUCTION: LMRReduction =
    unsafe { std::mem::transmute(*include_bytes!("bins/lmr_reduction.bin")) };

static DIAGONAL_LOOKUP: SliderLookup<NUM_DIAGONAL_ENTRIES> =
    unsafe { std::mem::transmute(*include_bytes!("bins/diagonal_lookup.bin")) };
