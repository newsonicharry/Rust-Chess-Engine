...
impl UCIParser {
    fn collect_until_end_or_breakpoint(
        start_index: usize,
    ...
    fn parse_set_option(split_message: Vec<&str>) -> Commands {
        ...
        for item in SPIN_OPTION_TABLE.iter() {
            if option_name == item.0 {
                option_type = OptionClassifier::Spin
            }
        }

        for item in BUTTON_OPTION_TABLE.iter() {
            if option_name == *item {
                option_type = OptionClassifier::Button
            }
        }
        ...
        if option_type == OptionClassifier::Spin {
            ...
            let value_wrapped =
                Self::collect_until_end_or_breakpoint(start_index, &split_message, None);
            ...
            return Commands::SetOption {
                options_type: OptionsType::Spin {
                    name: option_name,
                    value: u16::from_str(&value).unwrap(),
                },
            };
        }
        ...
        if option_type == OptionClassifier::Button {
            return Commands::SetOption {
                options_type: OptionsType::Button { name: option_name },
            };
        }
    ...
    fn parse_position(split_message: Vec<&str>) -> Commands {
        ...
        match split_message.get(1) {
            Some(&"fen") => {
                let unwrapped_fen =
                    Self::collect_until_end_or_breakpoint(2, &split_message, Some("moves"));
        ...
        if moves_position + 1 == split_message.len() {
            println!(
                "Command position did not include a value for the moves parameter, ignoring moves."
            );
            return Commands::Position {
                fen: fen.to_string(),
                moves: None,
        ...
        let moves = Some(
            split_message[moves_position + 1..]
                .iter()
                .map(|x| x.to_string())
                .collect::<Vec<String>>(),
        );
    ...
    fn parse_go(split_message: Vec<&str>) -> Commands {
        ...
        for i in (0..split_message.len() - 1).skip(1).step_by(2) {
            ...
            if Self::message_is_u32(message_value).is_err() {
                return Commands::IncorrectFormat;
            }
