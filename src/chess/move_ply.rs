...
impl MovePly {
    pub fn new(from: Square, to: Square, flag: MoveFlag) -> Self {
        let packed_data = ((flag as u16) << FLAG_SHIFT) | ((to as u16) << TO_SHIFT) | from as u16;
...
impl Display for MovePly {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        ...
        let final_str = from_file.to_string()
            + &*from_rank.to_string()
            + &*to_file.to_string()
            + &*to_rank.to_string()
            + &*promotion_piece;
...
pub fn uci_move_parser(played: &str, board: &Board) -> MovePly {
